	...
	public :
		...
		void addlightsaber(string str){
			lightsaber = (char*)str.c_str();		
                        ...
                        // lightsaber은 이 함수가 끝나면 dangling pointer
		}
...
class Kasa : public Ghost {
	public :
		...
		Kasa(int ghostage,string ghostname,string ghostmsg){
			...
			strcpy(name,ghostname.c_str());
			msg = ghostmsg ;
                        // uninitialized variable : foot => can be used for leakage vul
                        // 객체중에 Kasa랑 동일한 크기의 객체가 없고, 
                        // char[], String뿐인데, foot이 int이기 때문에 입력된 값 그대로 출력되서 의미없음.
		};
		...
		void addinfo(int ghostfoot,string str){
			...
			size = read(0,buf,20);
			buf[size] = 0;
                        // off-by-one : 1byte null  => need to check in binary 
                        //  => 최적화되면서 buf가 32byte 크기로 할당되서 문제없음.
			cout << "echo :" << buf << endl ;	
		};
...
void night(){
	...
	for(iter = ghostlist.begin();iter != ghostlist.end() ; iter++){
		num = 31+rand()%7;
		cout << "\033[" <<  num << "m";
		// iter는 (Ghost **) 타입이기에 speaking Templete함수에서
		// 의도와 다르게 항상 Ghost 의 speak() 함수가 호출된다.
		speaking(**iter);
		cout << "\033[0m";
	}
}
